...
1. Si ya ejecutaste el SQL inicial de GETS, ejecuta solamente `supabase/add_comment_targets.sql` en **SQL Editor**. Agrega la asociación de cada comentario con su foto o artículo sin borrar datos. Para instalaciones nuevas, `supabase/gallery_comments.sql` ya incluye todo.
...
El componente de comentarios usa `content_type` y un `content_id` estable para mantener separados los comentarios de cada foto o artículo. Al añadir contenido en el futuro, coloca `CommentsSection` con un ID nuevo. Los comentarios se limitan a 1000 caracteres y solo se aceptan de cuentas autenticadas. Las políticas RLS hacen cumplir la aprobación desde Supabase. No hay claves de Supabase ni cuentas de alumnas en el repositorio.
